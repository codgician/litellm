...
@pytest.mark.parametrize(
    ...
    [
        ("github_copilot/gemini-3.6-flash", 936000, 64000),
        ("github_copilot/gemini-3.7-flash", 936000, 64000),
        ("github_copilot/gemini-3.8-flash", 983040, 65536),
    ],
)
def test_github_copilot_gemini_flash_pricing(model: str, max_input_tokens: int, max_output_tokens: int):
    os.environ["LITELLM_LOCAL_MODEL_COST_MAP"] = "True"
    litellm.model_cost = litellm.get_model_cost_map(url="")
...
def test_github_copilot_gpt_6_astra_pricing():
    os.environ["LITELLM_LOCAL_MODEL_COST_MAP"] = "True"
    litellm.model_cost = litellm.get_model_cost_map(url="")
    ...
    assert model_info["litellm_provider"] == "github_copilot"
    assert model_info["mode"] == "responses"
    assert model_info["max_input_tokens"] == 872000
    assert model_info["max_output_tokens"] == 128000
    assert model_info["input_cost_per_token"] == 1e-5
    assert model_info["cache_read_input_token_cost"] == 1e-6
    assert model_info["cache_creation_input_token_cost"] == 1.25e-5
    assert model_info["output_cost_per_token"] == 5e-5
    assert model_info["input_cost_per_token_above_272k_tokens"] == 2e-5
    assert model_info["cache_read_input_token_cost_above_272k_tokens"] == 2e-6
    assert model_info["cache_creation_input_token_cost_above_272k_tokens"] == 2.5e-5
    assert model_info["output_cost_per_token_above_272k_tokens"] == 7.5e-5
    assert model_info["supported_endpoints"] == ["/v1/responses"]
    assert model_info["supports_response_schema"] is True
    assert model_info["supports_vision"] is True
    assert model_info["supports_pdf_input"] is True
    ...
    assert prompt_usd == pytest.approx((272800 * 2e-5) + (200 * 2e-6))
    assert completion_usd == pytest.approx(500 * 7.5e-5)
